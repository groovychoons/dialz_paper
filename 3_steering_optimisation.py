...
# Read in all 3 files
gender = pd.read_json('./data/bbq/BBQ_Gender_identity.jsonl', lines=True).head(100)
race = pd.read_json('./data/bbq/BBQ_Race_ethnicity.jsonl', lines=True).head(100)
...
def objective(trial, bbq_df):
    ...
    coeff = trial.suggest_float("coeff", -5, 5)
...
def run_bayes_optimization(bbq_df):
    ...
    study.optimize(lambda t: objective(t, bbq_df), n_trials=10)  # e.g. 20 trials to test

    fieldnames = ["Trial", "Accuracy", "Params"]  # Define CSV columns
